<?php
	...
	$app->post('/login', function()  use($app, $db) {
		...
		$sql = 	"SELECT c.idCuenta id, c.nombre FROM sia_cuentas c inner join sia_cuentausuario cu  on c.idCuenta=cu.idCuenta WHERE cu.predeterminada='SI' and cu.idUsuario =:idx  LIMIT 1";			
		$dbQuery = $db->prepare($sql);		
		...
		if($result){			
			...
			$sql = 	"SELECT idPrograma FROM sia_programas WHERE idCuenta=:cta  LIMIT 1";			
			$dbQuery = $db->prepare($sql);		
	...
	$app->get('/acopio', function() use($app, $db){
		...
		"DATE_FORMAT(ac.fAlta,'%d-%m-%Y') fecha, ac.idFase fase, ac.tipoArchivo, ac.estatus " . 
		"FROM sia_acopio ac " .  
	...
	$app->get('/papeles', function()  use ($app, $db) {
		...
		"DATE_FORMAT(p.fAlta,'%d-%m-%Y') fechaPapel, p.idFase fase, p.tipoPapel, p.tipoResultado, p.resultado, p.estatus  " .
		"FROM sia_papeles p   " .
	...
	$app->get('/cargarArchivo/:nombre', function($nombre)    use($app, $db) {		
		...
		$sql="DELETE FROM sia_pruebas WHERE idCuenta= :cuenta ;";				
		$dbQuery = $db->prepare($sql);
		$dbQuery->execute(array(':cuenta' => $cuenta));
		
		//Carga nuevaos datos para la cuenta pública
		$sql="INSERT INTO sia_pruebas (idCuenta, idSujeto, idObjeto, importeOriginal, usrAlta, fAlta, estatus) values(:cuenta, :sujeto, :objeto, :original, :usrActual, now(), 'ACTIVO');";				
		$dbQuery = $db->prepare($sql);			
		$result="OK";
		$nRegistros=0;
		for ($i = 1; $i <= $data->sheets[0]['numRows']; $i++) {		
			$sujeto = $data->sheets[0]['cells'][$i][1];
			$objeto = $data->sheets[0]['cells'][$i][2];
			$original = $data->sheets[0]['cells'][$i][3];
			$dbQuery->execute(array(':cuenta' => $cuenta, ':sujeto' => $sujeto, ':objeto' => $objeto, ':original' => $original, ':usrActual' => $usrActual));		
			$nRegistros++;
			//$result = $result . "Sujeto: " . $sujeto . " Objeto: " . $objeto . " Importe Original: " . $original;
		}
		echo "REgistro(s): " . $nRegistros;
