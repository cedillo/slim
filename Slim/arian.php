...
//guardar actividad auditor
	$app->post('/guardar/auditoria/actividad',  function()  use($app, $db) {
		...
		$auditoria = $request->post('auditoria');
		$idActivi = $request->post('IDActividad');

		$faseActividad=$request->post('FaseActividad');
		$Apartado=$request->post('Apartado');
		$DescripcionActividad=$request->post('DescripcionActividad');
		$preactivi=$request->post('txtPreviaActividad');
		...
		$diasefec=$request->post('diasefec');


		$porcen=$request->post('porcen');
		$PrioridadActividad=$request->post('PrioridadActividad');
		$tImpactoActividad=$request->post('tImpactoActividad');
		$ResponsableActividad=$request->post('ResponsableActividad');
		$EstatusActividad=$request->post('EstatusActividad');
		$NotasActividad= strtoupper($request->post('NotasActividad'));	
		
		try{
			if($INS=='INS'){
					$sql="INSERT INTO sia_auditoriasactividades (" .
					"idCuenta, idPrograma, idAuditoria, idFase, idTipo, fInicio,fFin, porcentaje, idPrioridad, idImpacto, notas, idResponsable, usrAlta, fAlta, estatus,diasEfectivos, actividad,idApartado) " .
					"values(:cuenta, :programa, :auditoria, :fase,'SIMPLE',:inicio, :fin,:porcentaje, :prioridad, :impacto,:notas,:responsable,:usrAlta,getdate(),'ACTIVO',:dactivi,:actividad,:idapartado)";

					$dbQuery = $db->prepare($sql);

					$dbQuery->execute(array(':cuenta' => $cuenta,':programa' => $programa,':auditoria' => $auditoria,':fase' => $fase,':inicio' => $inicio,':fin' => $fin,':porcentaje' => $porcentaje,':prioridad' => $prioridad,':impacto' => $impacto,':notas' => $notas,':responsable' => $responsable,':usrAlta' => $usrActual,':dactivi' => $dactivi,':actividad' => $actividad,':idapartado' => $idapartado));

					echo "GLOBAL PRUEBA <hr> $sql <br> <br>:cuenta=$cuenta  programa=$programa auditoria=$auditoria fase=$fase inicio=$inicio fin=$fin porcentaje=$porcentaje  prioridad=$prioridad impacto=$impacto notas=$notas responsable =$responsable usrAlta=$usrActual activi=$activi actividad=$actividad idapartado=$idapartado";

					}else{
						$sql="INSERT INTO sia_auditoriasactividades (" .
					"idCuenta, idPrograma, idAuditoria, idFase, idTipo, fInicio,fFin, porcentaje, idPrioridad, idImpacto, notas, idResponsable, usrAlta, fAlta, estatus,diasEfectivos, actividad,idApartado) " .
					"values(:cuenta, :programa, :auditoria, :faseActividad,'SIMPLE',:finicio, :fFin,:porcen, :PrioridadActividad, :tImpactoActividad,:NotasActividad,:ResponsableActividad,:usrAlta,getdate(),'ACTIVO',:diasefec,:DescripcionActividad,:Apartado)";

					$dbQuery = $db->prepare($sql);

					$dbQuery->execute(array(':cuenta' => $cuenta,':programa' => $programa,':auditoria' => $auditoria,':faseActividad' => $faseActividad,':finicio' => $finicio,':fFin' => $fFin,':porcen' => $porcen,':PrioridadActividad' => $PrioridadActividad,':tImpactoActividad' => $tImpactoActividad,':NotasActividad' => $NotasActividad,':ResponsableActividad' => $ResponsableActividad,':usrAlta' => $usrActual,':diasefec' => $diasefec,':DescripcionActividad' => $DescripcionActividad,':Apartado' => $Apartado));

					echo "GLOBAL PRUEBA111 <hr> $sql <br> <br>:cuenta=$cuenta  programa=$programa auditoria=$auditoria faseActividad=$faseActividad finicio=$finicio fFin=$fFin porcen=$porcen  PrioridadActividad=$PrioridadActividad tImpactoActividad=$tImpactoActividad NotasActividad=$NotasActividad ResponsableActividad =$ResponsableActividad usrAlta=$usrActual diasefec=$diasefec DescripcionActividad=$DescripcionActividad Apartado=$Apartado";
