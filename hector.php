...
//		$dbQuery->execute(array(':cuenta' => $cuenta));
	...
	$app->get('/lstUnidades', function()    use($app, $db) {
		...
		$sql="SELECT ltrim(idUnidad) id, nombre texto FROM sia_unidades WHERE ORDER BY nombre;";
...
//		$dbQuery->execute(array(':cuenta' => $cuenta, ':programa' => $programa, ':auditoria' => $auditoria, ':cuenta1' => $cuenta1, ':programa1' => $programa1));
	...
	$app->get('/guardar/auditoriaCriterios/:oper/:cadena', function($oper, $cadena)  use($app, $db) {
		...
		try
		{
			if ($cadena<>"")
			{
				...
				$auditoria=$campos [0];
				$criterio=$campos [1];
				$justificacion=strtoupper($campos [2]);
				$elementos=strtoupper($campos [3]);
	...
	*/
	...
	$app->get('/recuperaUnidadesByAuditoria/:auditoria', function($auditoria)  use($app, $db) {
		...
		$sql = "SELECT u.idSector + '|' + u.idSubsector + '|' + u.idUnidad id, u.nombre texto FROM sia_auditoriasunidades au INNER JOIN sia_unidades u ON au.idCuenta = u.idCuenta AND au.idSector = u.idSector 
				AND au.idSubsector = u.idSubsector AND au.idUnidad = u.idUnidad WHERE au.idCuenta = :cuenta AND au.idPrograma = :programa AND au.idAuditoria = :auditoria ORDER BY u.nombre ASC ";
	...
	$app->get('/lstNiveles', function()    use($app, $db) {
		$sql="SELECT distinct idNivel id, concat('NIVEL ', idNivel) texto FROM sia_empleados ORDER BY  1";
	...
	$app->post('/guardar/usuario_HVS', function()  use($app, $db) {
		...
		try {
    		...
    		$datos = 'oper: ' . $oper . '<br>id: ' . $id . '<br>tipo: ' . $tipo . '<br>empleado: '.$empleado.'<br>saludo: '.$saludo.'<br>nombre: ' . $nombre . '<br>paterno: ' . $paterno . '<br>materno: ' . $materno . '<br>area: ' . $area . '<br>correo: ' . $correo . '<br>Nivel: ' . $nivel . '<br>telefono: ' . $telefono . '<br>usuario: ' . $usuario . '<br>pwd: ' . $pwd . '<br>cuenta: ' . $cuenta . '<br>estatus: ' . $estatus . '<br>rolesUsuario: '. $rolesUsuario ;
			...
			if ($oper=='INS'){
				...
				if($total > 0){
					...
					$dbQuery = $db->prepare("UPDATE sia_empleados SET idNivel=:nivel WHERE idEmpleado=:empleado ");	
					$dbQuery->execute(array(':nivel'=> $nivel, ':empleado'=> $empleado));		

				}else{		

					$sql = "INSERT INTO sia_empleados (idEmpleado, nombre, paterno, materno, idArea, idNivel, usrAlta, fAlta, estatus) ".
					"VALUES(:empleado, :nombre, :paterno, :materno, :area, :nivel, :usrActual, getdate(), :estatus); ";		
					$dbQuery = $db->prepare($sql);	
					
					$dbQuery->execute(array(':empleado'=> $empleado, ':nombre'=> $nombre, ':paterno'=> $paterno, ':materno'=> $materno, ':area' => $area, ':nivel'=> $nivel, ':usrActual'=> $usrActual, ':estatus'=> $estatus));		
				}
	...
	$app->get('/catRoles', function()   use($app, $db) {
	...
	$app->post('/guardar/modulo_HVS', function()  use($app, $db) {
		...
		$app->redirect($app->urlFor('listaModulos'));
	});	
	...
	$app->get('/doctos', function() use($app, $db){
		$cuenta = $_SESSION["idCuentaActual"];
		$area = $_SESSION["idArea"];
	...
	$app->post('/guardar/documento', function()  use($app, $db) {
		...
		$app->redirect($app->urlFor('listaDocumentos'));
	});	
...
// ****  CATÁLOGO DE DÍAS INHABILES UNIDADES o ENTES *****
	...
	$app->get('/catInhabilesUnidad', function()  use ($app, $db) {
		$cuenta = $_SESSION["idCuentaActual"];
		$area = $_SESSION["idArea"];
...
//Guarda un día inhábil
	...
	$app->post('/guardar/inhabilesUnidad', function()  use($app, $db) {
		...
		$app->redirect($app->urlFor('listaInhabilesUnidad'));
	});
	...
	$app->get('/lstAsuntos', function()  use ($app, $db) {
		$cuenta = $_SESSION["idCuentaActual"];
		$area = $_SESSION["idArea"];
		$usrActual = $_SESSION["idUsuario"];
...
$app->get('/lstUsuariosExternosByEntidadExt/:laArea', function($laArea)  use ($app, $db) {
	...
	$app->post('/guardar/asunto', function()  use($app, $db) {
		...
		$app->redirect($app->urlFor('listaAsuntos'));
	});
	...
	$app->post('/guardar/tipoAuditoria', function()  use($app, $db) {
		...
		$nombre = $request->post('txtNombreTipoAuditoria');
		$estatus = $request->post('txtEstatus');
		...
		$app->redirect($app->urlFor('listaTiposAuditorias'));
	});
	...
	$app->get('/catPuestos', function()   use($app, $db) {
	...
	$app->get('/puestosEmpleados', function()   use($app, $db) {
	...
	$app->post('/guardar/puesto', function()  use($app, $db) {
		...
		$app->redirect($app->urlFor('listaPuestos'));
	});
	...
	$app->post('/guardar/puestoEmpleado', function()  use($app, $db) {
		...
		$app->redirect($app->urlFor('listaPuestosEmpleados'));
	});

	$app->get('/catTiposCriterios', function()   use($app, $db) {
	...
	$app->post('/guardar/tipoCriterio', function()  use($app, $db) {
		...
		$app->redirect($app->urlFor('listaTiposCriterios'));
	});
	...
	$app->get('/criteriosSeleccion', function() use($app, $db) {
	...
	$app->post('/guardar/criterioSeleccion', function()  use($app, $db) {
		...
		//echo( "oper=>" . $oper . " idCriterio=>" . $idCriterio  . " idTipoCriterio=>" . $idTipoCriterio . " fuente=>" . $fuente . " fecInformacion=>" . $fecInformacion . " estatus=>" . $estatus . " cuenta=>" . $cuenta . " programa=>" . $programa . " usrActual=>" . $usrActual);
		...
		try{
			...
			if($oper == 'INS'){
				...
				/*
				$sql="INSERT INTO sia_CriteriosAuxiliares (idCuenta, idPrograma, idTipoCriterio, fuente, informacion, fAlta, usrAlta) VALUES (:cuenta, :programa, :idTipoCriterio, :fuente, :informacion, getdate(), :usrAlta ) ;";
										
				$dbQuery = $db->prepare($sql);
				$dbQuery->execute(array(':cuenta'=>$cuenta, ':programa'=>$programa, ':idTipoCriterio'=>$idTipoCriterio, ':fuente'=>$fuente, ':informacion'=>$informacion, ':usrAlta'=> $usrActual));
				*/
				...
				$dbQuery = $db->prepare($sql);
				$dbQuery->execute(array(':idTipoCriterio'=>$idTipoCriterio, ':fuente'=>$fuente, ':informacion'=>$informacion, ':fecInformacion'=>$fecInformacion, ':usrModificacion'=>$usrActual, ':estatus'=>$estatus, ':idCuenta'=>$cuenta, ':idPrograma'=>$programa, ':idCriterioAuxiliar'=>$idCriterio));
		...
		$app->redirect($app->urlFor('listaCriteriosSeleccion'));
	});
	...
	$app->get('/catUnidades', function() use($app, $db) {
		//$cuenta = $_SESSION["idCuentaActual"];
	...
	$app->get('/actualizaVarSession/:cuentaSeleccionada', function($cuentaSeleccionada) use($app) {
		...
		//$app->redirect($app->urlFor('listaUnidades'));
	});
	
	//$app->get('/catUnidades2', function () use ($app) { $app->redirect($app->urlFor('listaUnidades',[,,,,]) );  });
	

	$app->get('/sectoresByCta/:idCuenta', function($idCuenta)    use($app, $db) {

		$sql="SELECT ltrim(idSector) id, ltrim(nombre) texto FROM sia_sectores Where idCuenta=:idCuenta ORDER BY nombre";
		$dbQuery = $db->prepare($sql);
	...
	$app->get('/subSectoresByCtaYsector/:idCuenta/:idSector', function($idCuenta, $idSector)    use($app, $db) {
		...
		$sql="SELECT ltrim(idSubsector) id, ltrim(nombre) texto FROM sia_subsectores Where idCuenta=:idCuenta and idSector=:idSector ORDER BY nombre";
		$dbQuery = $db->prepare($sql);
	...
	$app->get('/obtenerUnidadByCentroGestor/:idCuenta/:idSector/:idSubsector/:idUnidad', function($idCuenta, $idSector, $idSubsector, $idUnidad)    use($app, $db) {
		...
		$sql="SELECT idCuenta, idSector, idSubsector, idUnidad, isnull(nombre,'') nombre, isnull(siglas,'') siglas, isnull(titular,'') titular, isnull(estatus,'') estatus, idUnidadSector, idUnidadPoder, idUnidadClasificacion FROM sia_unidades WHERE idCuenta=:idCuenta AND idSector=:idSector AND idSubsector=:idSubsector AND idUnidad=:idUnidad ORDER BY nombre;";
	...
	$app->post('/guardar/unidad', function()  use($app, $db) {
		...
		//$datos = "<br>$oper <br>$idCuenta <br>$idSector <br>$idSubsector <br>$idUnidad <br>$nombre <br>$titular <br>$idUnidadSector <br>$idUnidadPoder <br>$idUnidadClasificacion" ;
		...
		try
		{
			if($oper=='INS')
			{
				$sql="INSERT INTO sia_unidades (idCuenta, idSector, idSubsector, idUnidad, nombre, titular, usrAlta, fAlta, estatus, idUnidadSector, idUnidadPoder, idUnidadClasificacion, siglas) " .
				"VALUES(:idCuenta, :idSector, :idSubsector, :idUnidad, :nombre, :titular, :usrAlta, getdate(), 'ACTIVO', :idUnidadSector, :idUnidadPoder, :idUnidadClasificacion, :siglas);";
				$dbQuery = $db->prepare($sql);

				$dbQuery->execute(array(':idCuenta'=>$idCuenta, ':idSector'=>$idSector, ':idSubsector'=>$idSubsector, ':idUnidad'=>$idUnidad, ':nombre'=>$nombre, ':titular'=>$titular, ':usrAlta'=>$usrActual, ':idUnidadSector'=>$idUnidadSector, ':idUnidadPoder'=>$idUnidadPoder, ':idUnidadClasificacion'=>$idUnidadClasificacion, ':siglas'=>$siglas ));
				//echo "<br>INS OK<hr>";
			}else{

				$sql="UPDATE sia_unidades SET nombre=:nombre, titular=:titular, usrModificacion=:usrModificacion, fModificacion=getdate(), estatus=:estatus, idUnidadSector=:idUnidadSector, idUnidadPoder=:idUnidadPoder, idUnidadClasificacion=:idUnidadClasificacion, siglas=:siglas WHERE idCuenta=:idCuenta AND idSector=:idSector AND idSubsector=:idSubsector AND idUnidad=:idUnidad ";
				$dbQuery = $db->prepare($sql);

				$dbQuery->execute(array(':nombre'=>$nombre, ':titular'=>$titular, ':usrModificacion'=>$usrActual, ':estatus'=>$estatus, ':idCuenta'=>$idCuenta, ':idSector'=>$idSector, ':idSubsector'=>$idSubsector, ':idUnidad'=>$idUnidad, ':idUnidadSector'=>$idUnidadSector, ':idUnidadPoder'=>$idUnidadPoder, ':idUnidadClasificacion'=>$idUnidadClasificacion, ':siglas'=>$siglas ));
				//echo "<br>UPD OK";
			}
	...
	// **************************** Rutinas para la CONSULTA del PGA ********************************************
		...
		$app->get('/consultapga', function()  use($app, $db) {
		$cuenta = $_SESSION["idCuentaActual"];
		$area = $_SESSION["idArea"];
		...
		if ($area == 'CAAAF' || $global ) {
			...
			$sql="SELECT a.idAuditoria auditoria, COALESCE(convert(varchar(20),a.clave),convert(varchar(20),a.idAuditoria)) claveAuditoria, isnull(a.clave,'') clave, ar.nombre area, dbo.lstSujetosByAuditoria(a.idAuditoria) sujeto, dbo.lstObjetosByAuditoria(a.idAuditoria) objeto, ta.nombre tipo, '0.00' avances FROM sia_programas p INNER JOIN sia_auditorias a on p.idCuenta=a.idCuenta and p.idPrograma=a.idPrograma 
	 			INNER JOIN sia_areas ar on a.idArea=ar.idArea  LEFT JOIN sia_tiposauditoria ta on a.tipoAuditoria= ta.idTipoAuditoria WHERE a.idCuenta=:cuenta AND a.idArea in (SELECT idArea from sia_areas where antecesor = :area) ORDER BY a.idAuditoria desc";
	...
	$app->get('/obtnerAuditoriasParaIntegracion/:idProceso/:idEtapa', function($idProceso, $idEtapa)  use($app, $db) {
