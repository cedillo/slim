...
>>>>>>> origin/master
	...
	$app->get('/catInhabiles', function()  use ($app, $db) {
		...
		$sql="SELECT dh.idCuenta idCta, dh.idDia idDia, dh.tipo tipo, dh.nombre nombre, CONVERT(VARCHAR(12),dh.finicio,102) fecIni, CONVERT(VARCHAR(12),dh.ffin,102) fecFin, dh.Estatus estatus FROM sia_diasinhabiles dh ORDER BY  dh.idCuenta, dh.idDia DESC";				
		$dbQuery = $db->prepare($sql);		
//		$dbQuery->execute(array(':cuenta' => $cuenta));
		$dbQuery->execute();
		$result['datos'] = $dbQuery->fetchAll(PDO::FETCH_ASSOC);
