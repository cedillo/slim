  ...
  <div class="col-md-8" style="padding:0px"> 
		...
		<?php
			...
			foreach($result as $row) {
				...
				echo "<tr><td colspan=2><b>DIRECCIÓN GENERAL</b><br>".$row['sArea']."</td><td colspan=2><b>DIRECCIÓN ÁREA</b><br>".$row['sResponsable'] . "</td><td colspan=2><b>SUBDIRECCIÓN</b><br>".$row['sSubresponsable']."</td></tr>";		
				echo "<tr><td colspan=4><b>SUJETO FISCALIZADO</b><br>".$row['sSujeto']."</tdcolspan=2><td><b>TIPO DE AUDITORÍA</b><br>".$row['sTipo'] . "</td></tr>";
				echo "<tr><td colspan=4><b>RUBRO O FUNCIÓN DE GASTO (O SU EQUIVALENTE)</b><br><p>".$row['sObjeto']."</p></td><tdcolspan=2><b>CLAVE</b><br>".$row['claveAuditoria'] . "</td></tr>";								
				


				/*$sql = "SELECT TOP 1 ((SELECT SUM(convert(INT,diasEfectivos)) FROM sia_auditoriasactividades where idAuditoria=:auditoria AND idFase='PLANEACION' )+(SELECT SUM(convert(INT,diasEfectivos)) FROM sia_auditoriasactividades where idAuditoria=:auditoria AND idFase='EJECUCION')+(SELECT SUM(convert(INT,diasEfectivos)) FROM sia_auditoriasactividades where idAuditoria=:auditoria AND idFase='INFORMES')) AS total FROM sia_auditoriasactividades where idAuditoria=:auditoria;";
				$dbQuery = $db->prepare($sql);		
				$dbQuery->execute(array(':auditoria' => $p2));
				$rs = $dbQuery->fetchAll(PDO::FETCH_ASSOC);

				foreach($rs as $row1) {*/
				echo "<tr><td rowspan=3  colspan=2><b>OBJETIVO</b><p>objetivo</p></td><td colspan=2><b>TOTAL DE DÍAS HÁBILES</b></td></tr>";
				echo "<tr><td><b>ORIGINAL</b></td><td><b>MODIFICADO</b></td></tr>";
				echo "<tr><td><b>52</b></td><td><b>52</b></td></tr>";
				//}
				echo "</table>";
				...
				foreach($rs2 as $row2) {
					echo "<tr><td width='40%'><b>".$row2['sFase']."</b></td><td width='10%'>".$row2['desde']."</td><td width='10%'>".$row2['desde']."</td><td width='10%'>".$row2['desde']."</td><td width='10%'>".$row2['desde']."</td><td width='10%'>".$row2['hasta']."</td><td width='10%'>".$row2['cantidad']."</td></tr>";				
				}
